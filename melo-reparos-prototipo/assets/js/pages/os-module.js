window.MeloOSModule = (() => {
  ...
  function detailHeader(os) { const v = vehicle(os), cl = client(os); const tipo = os.tipoAtendimento?.toLowerCase().includes('segur') ? 'Seguro' : os.tipoAtendimento?.toLowerCase().includes('loj') ? 'Lojista' : 'Particular'; return `<section class="card os-detail-head strong-os-header"><div class="os-title-block"><span class="eyebrow">${os.numero || os.id}</span><strong class="plate large">${v.placa}</strong><p>${v.marca || ''} ${v.modelo} · ${v.cor || '—'} · ${cl.nome}</p></div><div><span class="eyebrow">Status administrativo</span>${badge(os.status)}</div><div><span class="eyebrow">Tipo</span><strong>${tipo}</strong></div><div><span class="eyebrow">Seguro</span><strong>${tipo === 'Seguro' ? (os.seguradora || '—') : 'Não se aplica'}</strong></div><div><span class="eyebrow">Etapa atual</span><span class="badge primary">${stage(os).nome}</span></div><div><span class="eyebrow">Condições paralelas</span>${condBadges(os)}</div><div><span class="eyebrow">Previsão de entrega</span><strong>${fmtDate(os.previsao)} ${isOverdue(os) ? '<span class="badge danger">Atrasada</span>' : '<span class="badge success">No prazo</span>'}</strong></div><div><span class="eyebrow">Valor total</span><strong>${money(os.valor)}</strong></div><div><span class="eyebrow">Dias na oficina</span><strong>${daysBetween(os.entrada)} dias</strong></div><div class="os-discreet-actions"><a class="btn btn-secondary" href="ordens-servico.html">Voltar</a>${btnAction('Movimentar etapa','mover',os.id,'btn btn-primary')}${btnAction('Observação','observacao',os.id,'btn btn-secondary')}<button class="btn btn-secondary" onclick="window.print()">Imprimir</button></div></section>`; }
  ...
  function tabOverview(os) { const obs = d().observacoesOS.filter((o)=>o.osId===os.id).slice(0,4); const hist = d().historicoOS.filter((h)=>h.osId===os.id).slice(0,7); const pecasPendentes=d().pecasOS.filter((p)=>p.osId===os.id&&!slug(p.situacao).includes('recebido')).length; const compPendente=d().complementos.filter((co)=>co.osId===os.id&&!['Concluído','Cancelado','Recusado','Aprovado'].includes(co.status)).length; const tintaPendente=(os.condicoes||[]).includes('CON-02')?1:0; return `<section class="section"><div class="grid grid-5 compact-kpis">${c().kpiCard({label:'Dias na oficina', value:daysBetween(os.entrada), note:'desde entrada real'})}${c().kpiCard({label:'Dias na etapa atual', value:daysBetween(os.etapaEntrada), note:stage(os).nome})}${c().kpiCard({label:'Peças pendentes', value:pecasPendentes, note:'itens não recebidos'})}${c().kpiCard({label:'Complemento pendente', value:compPendente?'Sim':'Não', note:compPendente?`${compPendente} em aberto`:'sem bloqueio'})}${c().kpiCard({label:'Tinta pendente', value:tintaPendente?'Sim':'Não', note:'controle visual'})}</div><div class="grid grid-2"><article class="card"><h3>Observações recentes</h3>${obs.map(obsCard).join('') || c().emptyState('Sem observações','Nenhuma observação cadastrada.')}</article><article class="card"><h3>Linha do tempo resumida</h3>${timeline(hist, true)}</article></div></section>`; }
