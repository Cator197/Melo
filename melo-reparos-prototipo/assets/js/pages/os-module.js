window.MeloOSModule = (() => {
  ...
  function renderList() {
    const actions = `<button class="btn btn-primary" data-open-modal="new-os">Nova OS</button><button class="btn btn-secondary" data-open-modal="import-os">Importar orçamento</button><button class="btn btn-secondary" data-os-export>Exportar</button>`;
    setContent(`${pageHero('Ordens de Serviço', 'Consulta e administração simulada das OS, com filtros, movimentações, complementos e ações operacionais.', actions)}
  ...
  function indicatorCards() {
    ...
    return `<div class="grid grid-3 os-indicators">${cards.map(([key, label, value, note]) => `<button class="card kpi-card kpi-link os-indicator" data-indicator="${key}"><div class="kpi-label">${label}</div><div class="kpi-value">${value}</div><div class="kpi-note">${note}</div></button>`).join('')}</div>`;
  }
  ...
  function tableView(items) {
    const rows = items.map((os) => {
      ...
      return [linkOS(os), `<strong>${v.placa}</strong>`, `${v.marca || ''} ${v.modelo}`, cl.nome, payer(os), `<span class="badge primary">${stage(os).nome}</span>`, condBadges(os), fmtDate(os.entrada), fmtDate(os.previsao), `${daysBetween(os.entrada)} dias`, badge(os.status), quickActions(os)];
    });
    return c().table(['OS','Placa','Veículo','Cliente','Pagador principal','Etapa principal','Condições paralelas','Entrada','Previsão','Dias oficina','Status','Ações'], rows);
  }
  function cardsView(items) {
    return `<div class="os-card-grid">${items.map((os) => { const v = vehicle(os), cl = client(os); return `<article class="card os-card"><div class="os-card-top"><div><a class="os-number" href="ordem-servico-detalhes.html?id=${os.id}">${os.numero || os.id}</a><strong class="plate">${v.placa}</strong></div>${badge(os.status)}</div><p>${v.marca || ''} ${v.modelo} · ${cl.nome}</p><div class="os-card-lines"><span>Etapa</span><b>${stage(os).nome}</b><span>Condições</span><div>${condBadges(os)}</div><span>Previsão</span><b>${fmtDate(os.previsao)} ${isOverdue(os) ? '<span class="badge danger">Atrasada</span>' : '<span class="badge success">No prazo</span>'}</b><span>Valor aprovado</span><b>${money(os.valor)}</b></div><div class="os-actions-row">${quickActions(os)}</div></article>`; }).join('')}</div>`;
  }
  ...
  function detailHeader(os) { const v = vehicle(os), cl = client(os); return `${pageHero(os.numero || os.id, `${v.placa} · ${v.marca || ''} ${v.modelo} · ${cl.nome}`, `<a class="btn btn-secondary" href="ordens-servico.html">Voltar</a>${btnAction('Editar OS','editar',os.id,'btn btn-secondary')}${btnAction('Movimentar etapa','mover',os.id,'btn btn-primary')}${btnAction('Adicionar condição','condicao',os.id,'btn btn-secondary')}${btnAction('Adicionar complemento','complemento',os.id,'btn btn-secondary')}${btnAction('Adicionar documento','documento',os.id,'btn btn-secondary')}${btnAction('Adicionar foto','foto',os.id,'btn btn-secondary')}<button class="btn btn-secondary" onclick="window.print()">Imprimir</button><div class="more-actions"><button class="btn btn-secondary" data-more-toggle>Mais ações</button><div class="more-menu">${btnAction('Alterar previsão','previsao',os.id)}${btnAction('Adicionar observação','observacao',os.id)}${btnAction('Finalizar produção','finalizar',os.id)}${btnAction('Marcar entrega','entregar',os.id)}${btnAction('Fechar OS','fechar',os.id)}${btnAction('Cancelar OS','cancelar',os.id)}${btnAction('Reabrir OS','reabrir',os.id)}</div></div>`)}<section class="card os-detail-head"><div><span class="eyebrow">Placa</span><strong class="plate large">${v.placa}</strong></div><div><span class="eyebrow">Status administrativo</span>${badge(os.status)}</div><div><span class="eyebrow">Etapa principal</span><span class="badge primary">${stage(os).nome}</span></div><div><span class="eyebrow">Condições paralelas</span>${condBadges(os)}</div><div><span class="eyebrow">Previsão</span><strong>${fmtDate(os.previsao)} ${isOverdue(os) ? '<span class="badge danger">Atrasada</span>' : '<span class="badge success">No prazo</span>'}</strong></div><div><span class="eyebrow">Valor aprovado</span><strong>${money(os.valor)}</strong></div><div><span class="eyebrow">Dias na oficina</span><strong>${daysBetween(os.entrada)} dias</strong></div></section>`; }
  ...
  function tabs() { const items = [['visao','Visão geral'],['producao','Produção'],['servicos','Serviços e peças'],['complementos','Complementos'],['compras','Compras'],['financeiro','Financeiro'],['documentos','Documentos e fotos'],['historico','Histórico']]; return `<div class="tabs os-tabs">${items.map(([id,label]) => `<button class="tab" data-tab="${id}" type="button">${label}</button>`).join('')}</div>`; }
  function bindDetail(os) { document.querySelector('.content').addEventListener('input', (event) => { if (event.target.matches('[data-history-search], [data-history-type]')) filterHistory(os); }); document.querySelector('.content').addEventListener('change', (event) => { if (event.target.matches('[data-history-type]')) filterHistory(os); }); document.querySelector('.content').addEventListener('click', (event) => { const a = event.target.closest('[data-os-action]'); if (a) openActionModal(a.dataset.osAction, a.dataset.osId); const tab = event.target.closest('[data-tab]'); if (tab) renderTab(os, tab.dataset.tab); const close = event.target.closest('[data-close-condition]'); if (close) closeCondition(os, close.dataset.closeCondition); const pin = event.target.closest('[data-pin-obs]'); if (pin) togglePin(pin.dataset.pinObs, os); const photo = event.target.closest('[data-photo-view]'); if (photo) c().toast('Visualização de foto simulada aberta.'); const doc = event.target.closest('[data-doc-action]'); if (doc) c().toast(`${doc.dataset.docAction} simulado.`); const compra = event.target.closest('[data-compra-modal]'); if (compra) c().toast('Detalhe simplificado da compra aberto em modal simulado.'); }); document.querySelector('[data-action-modal]')?.addEventListener('click', (e)=>{ if(e.target.matches('[data-modal-close-custom]')) closeModals(); }); document.querySelector('[data-more-toggle]')?.addEventListener('click', () => document.querySelector('.more-menu')?.classList.toggle('is-open')); }
  function renderTab(os, tab) { sessionStorage.setItem(`melo-os-tab-${os.id}`, tab); document.querySelectorAll('[data-tab]').forEach((b) => b.classList.toggle('active', b.dataset.tab === tab)); document.querySelector('[data-tab-content]').innerHTML = ({ visao: tabOverview, producao: tabProduction, servicos: tabServices, complementos: tabComplements, compras: tabPurchases, financeiro: tabFinance, documentos: tabDocs, historico: tabHistory }[tab] || tabOverview)(os); }
  function tabOverview(os) { const obs = d().observacoesOS.filter((o)=>o.osId===os.id).slice(0,4); const hist = d().historicoOS.filter((h)=>h.osId===os.id).slice(0,7); return `<section class="section"><div class="grid grid-3">${c().kpiCard({label:'Dias na oficina', value:daysBetween(os.entrada), note:'desde entrada real'})}${c().kpiCard({label:'Dias na etapa', value:daysBetween(os.etapaEntrada), note:stage(os).nome})}${c().kpiCard({label:'Conclusão estimada', value:`${os.conclusao || 0}%`, note:'provisória'})}${c().kpiCard({label:'Pendências', value:(os.condicoes||[]).length, note:'condições ativas'})}${c().kpiCard({label:'Peças aguardando', value:d().pecasOS.filter((p)=>p.osId===os.id&&!slug(p.situacao).includes('recebido')).length, note:'itens'})}${c().kpiCard({label:'Complementos abertos', value:d().complementos.filter((co)=>co.osId===os.id&&!['Concluído','Cancelado','Recusado'].includes(co.status)).length, note:'processos paralelos'})}</div><div class="grid grid-2"><article class="card"><h3>Resumo operacional</h3><p><b>Etapa principal:</b> ${stage(os).nome}</p><p><b>Condições:</b> ${condBadges(os)}</p><p><b>Responsável:</b> ${os.responsavel}</p><p><b>Previsão:</b> ${fmtDate(os.previsao)}</p><p><b>Alertas:</b> ${os.alerta || 'Sem alerta crítico.'}</p><p><b>Próximo passo recomendado:</b> resolver condições paralelas antes de avançar etapa.</p></article><article class="card"><h3>Observações recentes</h3>${obs.map(obsCard).join('') || c().emptyState('Sem observações','Nenhuma observação cadastrada.')}</article></div><article class="card"><h3>Linha do tempo resumida</h3>${timeline(hist, true)}</article></section>`; }
  ...
  function tabComplements(os) { const comps = d().complementos.filter((co)=>co.osId===os.id); return `<section class="section"><div class="section-header"><h3 class="section-title">Complementos</h3>${btnAction('Novo complemento','complemento',os.id,'btn btn-primary')}</div><div class="grid grid-2">${comps.map((co)=>`<article class="card"><h3>${co.numero}</h3>${badge(co.status)}<p><b>Motivo:</b> ${co.motivo}</p><p>${co.descricao}</p><p><b>Solicitado:</b> ${money(co.valorSolicitado)} · <b>Aprovado:</b> ${money(co.valorAprovado)}</p><p><b>Impacto:</b> ${co.impactoPrevisao} · ${fmtDate(co.novaPrevisao)}</p><p><b>Itens:</b> ${(co.itens||[]).join(', ') || '—'}</p><p><b>Documentos:</b> ${(co.documentos||[]).join(', ') || '—'}</p><small>${co.observacoes}</small></article>`).join('')}</div></section>`; }
  function tabPurchases(os) { const purchases = d().compras.filter((co)=>co.osId===os.id); return `<section class="section"><article class="card"><h3>Compras vinculadas</h3>${c().table(['Pedido','Fornecedor','Item','Qtd. OS','Valor rateado','Situação','Recebido','Previsão','Atraso','Conta a pagar','Ação'], purchases.map((p)=>[p.pedido,byId(d().fornecedores,p.fornecedorId).nome,p.item,p.quantidade,money(p.valor),badge(p.situacao||p.status),`${p.quantidadeRecebida}/${p.quantidade}`,fmtDate(p.previsaoEntrega),p.previsaoEntrega<today&&p.quantidadeRecebida<p.quantidade?'<span class="badge danger">Atrasada</span>':'<span class="badge success">No prazo</span>',p.contaPagarId||'<span class="badge warning">Sem conta</span>',`<button class="btn btn-secondary" data-compra-modal="${p.id}">Abrir detalhe</button>`]))}</article></section>`; }
  function tabFinance(os) { const fin = d().financeiroOS?.[os.id] || { receitas: [{ pagador: os.seguradora, descricao:'Valor aprovado', bruto:os.valor, taxa:0, liquido:os.valor, vencimento:os.previsao, status:'Previsto' }], custos: { pecas: os.valores?.pecas||0, materiais: os.valores?.materiais||0, terceiros: os.valores?.terceiros||0, taxas:0, outros:0 } }; const receita = fin.receitas.reduce((s,r)=>s+r.bruto,0), liquida=fin.receitas.reduce((s,r)=>s+r.liquido,0), custos=Object.values(fin.custos).reduce((s,v)=>s+v,0); const provisional = d().compras.some((p)=>p.osId===os.id&&p.quantidadeRecebida<p.quantidade) || d().complementos.some((co)=>co.osId===os.id&&co.status.includes('Aguardando')); return `<section class="section"><div class="grid grid-4">${c().kpiCard({label:'Receita aprovada',value:money(receita),note:'bruta'})}${c().kpiCard({label:'Receita líquida prevista',value:money(liquida),note:'após taxas'})}${c().kpiCard({label:'Custos estimados',value:money(custos),note:'provisórios'})}${c().kpiCard({label:'Lucro estimado',value:money(liquida-custos),note:`Margem ${liquida?Math.round((liquida-custos)/liquida*100):0}%`})}</div>${provisional?'<div class="alert-inline">Valores provisórios: há compras pendentes, custos sem confirmação ou complemento aguardando aprovação. Lucro realizado não é definitivo.</div>':''}<article class="card"><h3>Receitas</h3>${c().table(['Pagador','Descrição','Valor bruto','Taxa','Valor líquido','Vencimento','Status'], fin.receitas.map((r)=>[r.pagador,r.descricao,money(r.bruto),money(r.taxa),money(r.liquido),fmtDate(r.vencimento),badge(r.status)]))}</article><article class="card"><h3>Custos</h3>${c().table(['Peças','Materiais','Terceiros','Taxas','Outros custos'], [[money(fin.custos.pecas),money(fin.custos.materiais),money(fin.custos.terceiros),money(fin.custos.taxas),money(fin.custos.outros)]])}</article></section>`; }
