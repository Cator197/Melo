window.MeloOSModule = (() => {
  ...
  function osChecklist(os) { const pecasPendentes = d().pecasOS.filter((p)=>p.osId===os.id&&!slug(p.situacao).includes('recebido')).length; const compPendente=d().complementos.filter((co)=>co.osId===os.id&&!['Concluído','Cancelado','Recusado','Aprovado'].includes(co.status)).length; const tintaPendente=(os.condicoes||[]).includes('CON-02')?1:0; return { pecasPendentes, compPendente, tintaPendente }; }
  ...
  function detailHeader(os) { const v = vehicle(os); const tipo = os.tipoAtendimento?.toLowerCase().includes('segur') ? 'Seguro' : os.tipoAtendimento?.toLowerCase().includes('loj') ? 'Lojista' : 'Particular'; const checks = osChecklist(os); const headerStatus = checks.pecasPendentes || checks.compPendente || checks.tintaPendente ? 'status-warning' : 'status-ok'; return `<section class="card os-detail-head strong-os-header ${headerStatus}">
    <div class="os-detail-title">
      <span>Detalhe da ordem de serviço</span>
      <h2>${v.marca || ''} ${v.modelo} - ${v.cor || '—'}</h2>
      <strong>${v.placa} - ${os.numero || os.id}</strong>
    </div>
  ...
  function statusKpi(label, pending, note) { return `<article class="card kpi-card os-status-kpi ${pending ? 'status-warning' : 'status-ok'}"><div class="kpi-label">${label}</div><div class="kpi-value">${pending ? 'Pendente' : 'OK'}</div><div class="kpi-note">${note}</div></article>`; }
  function tabOverview(os) { const obs = d().observacoesOS.filter((o)=>o.osId===os.id).slice(0,4); const checks = osChecklist(os); return `<section class="section"><div class="grid grid-4 compact-kpis">${statusKpi('Peças', checks.pecasPendentes, checks.pecasPendentes ? `${checks.pecasPendentes} item(ns) não recebido(s)` : 'sem peças pendentes')}${statusKpi('Complemento', checks.compPendente, checks.compPendente ? `${checks.compPendente} em aberto` : 'sem bloqueio')}${statusKpi('Tinta', checks.tintaPendente, checks.tintaPendente ? 'acompanhar autorização/insumo' : 'controle liberado')}${c().kpiCard({label:'Dias na etapa atual', value:daysBetween(os.etapaEntrada), note:stage(os).nome})}</div><article class="card observations-section"><h3>Observações</h3><div class="new-observation"><textarea class="textarea" placeholder="Digite uma nova observação"></textarea><button class="btn btn-secondary" type="button" data-toast-demo>Adicionar</button></div>${obs.map(obsCard).join('') || c().emptyState('Sem observações','Nenhuma observação cadastrada.')}</article></section>`; }
