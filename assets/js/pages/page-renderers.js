window.MeloPages = (() => {
  ...
  function renderInicio() {
    ...
    const alerts = homeAlertsV2();
    const topPriorities = alerts.filter((alert) => alert.priority === 'Alta').slice(0, 3);
    const supportingAlerts = alerts.slice(3, 7);
    const focusItems = [
      { label: 'OS ativas', value: carrosAtivos.length, note: 'Veiculos em andamento', action: 'Ver fila', href: 'ordens-servico.html' },
      { label: 'Atrasadas', value: atrasadas.length, note: 'Previsao vencida', action: 'Tratar agora', href: 'ordens-servico.html?filtro=atrasadas', tone: 'danger' },
      { label: 'Compras pendentes', value: comprasPendentes.length, note: 'Pecas e servicos', action: 'Priorizar', href: 'compras.html?status=pendente' },
      { label: 'Receber hoje', value: c().money(sum(receberHoje.map((item) => ({ valor: item.valorLiquido ?? item.valor ?? 0 })))), note: `${receberHoje.length} titulo(s)`, action: 'Cobrar', href: 'contas-receber.html?periodo=hoje', tone: 'success' },
      { label: 'Pagar hoje', value: c().money(sum(pagarHoje)), note: `${pagarHoje.length} obrigacao(oes)`, action: 'Programar', href: 'contas-pagar.html?periodo=hoje', tone: 'warning' }
    ];
    const agendaHoje = dayOperationBlocks().filter((block) => block.items.length);

    setContent(`
      <section class="section home-focus">
        <div>
          <span class="home-date">Hoje · ${formatDate(today)}</span>
          <h2>Central do dia</h2>
          <p>Comece pelos bloqueios que afetam entrega, producao e caixa. Os indicadores abaixo entram como suporte para decisao.</p>
        </div>
        <div class="home-focus-actions">
          <a class="btn btn-primary" href="agenda.html">Abrir agenda</a>
          <a class="btn btn-secondary" href="ordens-servico.html">Ver OS</a>
        </div>
      </section>
      <section class="section home-priority-grid" aria-label="Prioridades do dia">
        ${topPriorities.map(priorityCard).join('')}
      </section>
      <section class="section home-metrics" aria-label="Indicadores de apoio">${focusItems.map(kpiLink).join('')}</section>
      <section class="section home-workbench">
        <article class="card home-panel long-stage-highlight">
          <div class="section-header"><h3 class="section-title">Veiculos travados na etapa</h3><a class="btn btn-secondary btn-sm" href="producao.html?filtro=tempo-etapa">Producao</a></div>
          ${longStageTable(longas)}
        </article>
        <article class="card home-panel">
          <div class="section-header"><h3 class="section-title">Agenda de hoje</h3><a class="btn btn-secondary btn-sm" href="agenda.html">Agenda</a></div>
          <div class="home-agenda-list">${agendaHoje.length ? agendaHoje.map(operationBlock).join('') : '<p class="empty-mini">Nenhum compromisso operacional para hoje.</p>'}</div>
        </article>
      </section>
      <section class="section home-alerts">
        <div class="section-header"><h3 class="section-title">Alertas de acompanhamento</h3><a class="btn btn-secondary btn-sm" href="relatorios.html">Ver relatorios</a></div>
        <div class="alert-list compact-alerts">${supportingAlerts.map(alertCardV2).join('')}</div>
      </section>
    `);
  }
  ...
  function homeAlertsV2() {
    return [
      { priority: 'Alta', text: 'Entrega de OS-1005 esta prevista para hoje e ainda depende da terceirizacao.', href: 'ordens-servico.html?os=OS-1005', action: 'Tratar entrega', record: 'OS-1005', owner: 'Producao', due: 'Hoje', impact: 'Entrega em risco' },
      { priority: 'Alta', text: 'OS-1003 esta ha 9 dias na Preparacao, acima do limite esperado.', href: 'producao.html?os=OS-1003', action: 'Revisar etapa', record: 'OS-1003', owner: 'Rafael', due: 'Hoje', impact: 'Gargalo de producao' },
      { priority: 'Alta', text: 'REC-006 venceu em 08/06 e precisa de cobranca antes do fechamento.', href: 'contas-receber.html?rec=REC-006', action: 'Cobrar cliente', record: 'REC-006', owner: 'Financeiro', due: 'Vencido', impact: 'Caixa do dia' },
      { priority: 'Alta', text: 'PAG-006 venceu em 10/06 e deve ser programado.', href: 'contas-pagar.html?pag=PAG-006', action: 'Programar pagamento', record: 'PAG-006', owner: 'Financeiro', due: 'Vencido', impact: 'Fornecedor' },
      { priority: 'Media', text: 'Kit tinta azul perolizado chegou fora da data combinada e precisa de conferencia.', href: 'compras.html?compra=COM-002', action: 'Conferir compra', record: 'COM-002', owner: 'Compras', due: 'Hoje', impact: 'Pintura' },
      { priority: 'Media', text: 'Compra COM-004 nao possui conta a pagar vinculada.', href: 'compras.html?compra=COM-004', action: 'Gerar titulo', record: 'COM-004', owner: 'Compras', due: 'Hoje', impact: 'Lancamento financeiro' },
      { priority: 'Media', text: 'CMP-001 aguarda aprovacao para liberar a sequencia de montagem.', href: 'complementos.html?comp=CMP-001', action: 'Solicitar aprovacao', record: 'CMP-001', owner: 'Administrativo', due: 'Hoje', impact: 'Montagem' },
      { priority: 'Media', text: 'OS-1007 foi finalizada sem fechamento financeiro completo.', href: 'financeiro-visao-geral.html?os=OS-1007', action: 'Fechar financeiro', record: 'OS-1007', owner: 'Financeiro', due: 'Esta semana', impact: 'Rentabilidade' }
    ];
  }
  ...
  function operationBlock(block) {
    return `<article class="card op-block"><h4>${block.title}</h4>${block.items.length ? block.items.map((item) => `<a href="agenda.html?evento=${item.id}"><strong>${item.hora}</strong> ${item.titulo}<span>${item.status}</span></a>`).join('') : '<p class="empty-mini">Nenhum registro para hoje.</p>'}</article>`;
  }

  function priorityCard(alert, index) {
    const cls = alert.priority === 'Alta' ? 'danger' : 'warning';
    return `<article class="card priority-card ${cls}">
      <div class="priority-rank">${index + 1}</div>
      <div class="priority-body">
        <div class="alert-meta"><span class="badge ${cls}">${alert.priority}</span><small>${alert.record}</small></div>
        <h3>${alert.text}</h3>
        <dl><dt>Responsavel</dt><dd>${alert.owner}</dd><dt>Prazo</dt><dd>${alert.due}</dd><dt>Impacto</dt><dd>${alert.impact}</dd></dl>
        <a class="btn btn-primary btn-sm" href="${alert.href}">${alert.action}</a>
      </div>
    </article>`;
  }

  function alertCardV2(alert) {
    const cls = alert.priority === 'Alta' ? 'danger' : 'warning';
    return `<article class="card alert-card ${cls}"><div><div class="alert-meta"><span class="badge ${cls}">${alert.priority}</span><small>${alert.record}</small><small>${alert.owner}</small></div><h3 class="alert-title">${alert.text}</h3><p class="alert-text">${alert.impact} - prazo: ${alert.due}.</p><a class="btn btn-secondary" href="${alert.href}">${alert.action}</a></div></article>`;
  }
  ...
  function kpiLink(item) {
    ...
    return `<a class="card kpi-card kpi-link${tone}" href="${item.href}"><div class="kpi-label">${item.label}</div><div class="kpi-value">${item.value}</div><div class="kpi-note">${item.note}</div><span class="kpi-action">${item.action}</span></a>`;
  }
